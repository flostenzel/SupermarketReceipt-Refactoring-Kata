...
class SupermarketTest(unittest.TestCase):
    ...
    def test_two_normal_items(self):
        ...
        assert receipt.total_discount_amount() == 0
        assert receipt.total_price() == approx(float(0.99 + 2.99), rel=0.01)
        self.compare_with_html(receipt)
    ...
    def test_buy_two_get_one_free(self):
        ...
        self.the_cart.add_item(self.toothbrush)
        self.the_cart.add_item(self.toothbrush)
        self.teller.add_special_offer(SpecialOfferType.THREE_FOR_TWO, self.toothbrush, self.catalog.unit_price(product=self.toothbrush))
        receipt = self.teller.checks_out_articles_from(self.the_cart)
        assert receipt.total_item_price_amount() == 3 * 0.99
        assert receipt.total_discount_amount() == approx(-0.99, 0.01)
        assert receipt.total_price() == approx(2 * 0.99, 0.01)
        self.compare_with_html(receipt)
    ...
    def test_buy_five_get_one_free(self):
        ...
        self.teller.add_special_offer(SpecialOfferType.THREE_FOR_TWO, self.toothbrush, self.catalog.unit_price(product=self.toothbrush))
        receipt = self.teller.checks_out_articles_from(self.the_cart)
        assert receipt.total_item_price_amount() == approx(5 * 0.99, 0.01)
        assert receipt.total_discount_amount() == approx(-0.99, 0.01)
        assert receipt.total_price() == approx(4 * 0.99, 0.01)
        self.compare_with_html(receipt)
    ...
    def test_percent_discount(self):
        ...
        assert receipt.total_discount_amount() == approx(-2.99 / 10, 0.01)
        assert receipt.total_price() == approx(2.99 - 2.99 / 10, 0.01)
        self.compare_with_html(receipt)
    ...
    def test_x_for_y_discount(self):
        ...
        assert receipt.total_discount_amount() == approx(0.99 - (0.69 * 2), 0.01)
        assert receipt.total_price() == approx(0.99, 0.01)
        self.compare_with_html(receipt)
    ...
    def test_five_for_y_discount(self):
        ...
        receipt = self.teller.checks_out_articles_from(self.the_cart)
        assert receipt.total_item_price_amount() == 5 * 1.99
        assert receipt.total_discount_amount() == approx(5.99 - 5 * 1.99, 0.01)
        assert receipt.total_price() == approx(5.99, 0.01)
        self.compare_with_html(receipt)
    ...
    def test_five_for_y_discount_with_six(self):
        ...
        assert receipt.total_discount_amount() == approx(5.99 - 5 * 1.99, 0.01)
        assert receipt.total_price() == approx(5.99 + 1.99, 0.01)
        self.compare_with_html(receipt)
    ...
    def test_five_for_y_discount_with_sixteen(self):
        ...
        assert receipt.total_discount_amount() == approx(-5.88, 0.01)
        assert receipt.total_price() == approx(25.96, 0.01)
        self.compare_with_html(receipt)
